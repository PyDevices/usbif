...
# HID usage id -> SDL-style keycode, and the modifier byte -> KMOD mask. The
# table is pydevices' keys.py, the same one bledev.hidreport decodes BLE
# keyboards with, so an application comparing against keys.K_a sees the same
# key whether it came over USB, over Bluetooth or from SDL. There is no copy
# here to drift.
#
# Modifiers need all three of (KMOD mask, keycode, HID usage) because HID
# reports them only as a bitmask, never in the usage array -- so pressing
# Shift alone produces a report with no usages at all. SDL emits a KEYDOWN for
# the modifier key itself, and M1's promise is the same events as SDL, so
# these must be diffed and emitted too. Their usage ids (0xE0-0xE7) are what a
# keyboard would send if modifiers appeared in the array, and are the honest
# scancode to report.
_MODIFIER_KEYS = tuple(
    (1 << i, kmod, code, 0xE0 + i)
    for i, (kmod, code) in enumerate(keys.HID_MODIFIERS)
)
