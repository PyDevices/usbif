...
// --- Our own copy of TinyUSB's speaker descriptor, with one fix.
//
// TUD_AUDIO_SPEAKER_MONO_FB_DESCRIPTOR hardcodes bInterval = 1 on *both*
// isochronous endpoints. At full speed that is one frame (1 ms) and correct.
// At high speed a bInterval of 1 means one *microframe* -- 125 us -- and while
// that is fine for the data endpoint, a feedback endpoint refreshing 8000
// times a second is outside what the audio class intends and what Windows'
// usbaudio2 driver accepts: the function enumerates, binds usbaudio2, and then
// fails to start with Code 10.
//
// Verified by dumping the descriptor the board actually presents: every length
// agreed (declared 214 = actual = wTotalLength), interfaces and classes were
// correct, and the feedback endpoint was the single non-conforming field.
//
// Only the feedback interval is parameterised; everything else is TinyUSB's,
// kept verbatim so it stays easy to diff against upstream.
#define USBIF_AUDIO_SPEAKER_MONO_FB_DESCRIPTOR(_itfnum, _stridx, _nBytesPerSample, _nBitsUsedPerSample, _epout, _epoutsize, _epfb, _epfbsize, _epfbinterval) \
  ...
  TUD_AUDIO_DESC_STD_AS_ISO_FB_EP(/*_ep*/ _epfb, /*_epsize*/ _epfbsize, /*_interval*/ _epfbinterval)
...
#define MICROPY_HW_USB_EXT_DESC_CFG_LEN (TUD_AUDIO_SPEAKER_MONO_FB_DESC_LEN)
...
#define MICROPY_HW_USB_EXT_DESC_CFG                    \
    USBIF_AUDIO_SPEAKER_MONO_FB_DESCRIPTOR(            \
    ...
    /*_epfbsize*/ 4,                                   \
    /*_epfbinterval*/ USBIF_AUDIO_FB_INTERVAL),
